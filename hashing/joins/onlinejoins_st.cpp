 ...
 */
void
online_init_bucket_buffer(bucket_buffer_t **ppbuf) {
 ...
 */
static inline void
online_get_new_bucket(bucket_t **result, bucket_buffer_t **buf) {
...
void
online_free_bucket_buffer(bucket_buffer_t *buf) {
    do {
 ...
 */
void
online_allocate_hashtable(hashtable_t **ppht, uint32_t nbuckets) {
    hashtable_t *ht;
 ...
 */
void
online_destroy_hashtable(hashtable_t *ht) {
    free(ht->buckets);
    free(ht);
}
 ...
 */
void
online_build_hashtable_st(hashtable_t *ht, relation_t *rel) {
    uint32_t i;
 ...
 */
int64_t
online_probe_hashtable(hashtable_t *ht, relation_t *rel, void *output) {
    uint32_t i, j;
    int64_t matches;
...
SHJ_st(relation_t *relR, relation_t *relS, int nthreads) {
    ...
    online_allocate_hashtable(&ht, nbuckets);
...
#endif
    online_build_hashtable_st(ht, relR);
    ...
    result = online_probe_hashtable(ht, relS, chainedbuf);
 ...
 */
void
online_build_hashtable_mt(hashtable_t *ht, relation_t *rel,
                          bucket_buffer_t **overflowbuf) {
    uint32_t i;
    const uint32_t hashmask = ht->hash_mask;
    const uint32_t skipbits = ht->skip_bits;
    ...
    for (i = 0; i < rel->num_tuples; i++) {
        ...
        if (curr->count == BUCKET_SIZE) {
            if (!nxt || nxt->count == BUCKET_SIZE) {
                ...
                online_get_new_bucket(&b, overflowbuf);
                curr->next = b;
                b->next = nxt;
                b->count = 1;
                dest = b->tuples;
 ...
 */
void *
online_npo_thread(void *param) {
    int rv;
    arg_t *args = (arg_t *) param;

    /* allocate overflow buffer for each thread */
    bucket_buffer_t *overflowbuf;
    online_init_bucket_buffer(&overflowbuf);
    ...
    online_build_hashtable_mt(args->ht, &args->relR, &overflowbuf);
    ...
    args->num_results = online_probe_hashtable(args->ht, &args->relS, chainedbuf);
    ...
    online_free_bucket_buffer(overflowbuf);
...
SHJ(relation_t *relR, relation_t *relS, int nthreads) {
    ...
    for (i = 0; i < nthreads; i++) {
        ...
        rv = pthread_create(&tid[i], &attr, online_npo_thread, (void *) &args[i]);
        if (rv) {
