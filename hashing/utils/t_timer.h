...
struct T_TIMER {
    ...
    uint64_t overall_timer, simple_buildtimer, partition_timer, buildtimer_pre = 0, buildtimer = 0;//buildtimer is accumulated.
...
#define BEGIN_MEASURE_BUILD(timer) \
    startTimer(&timer.simple_buildtimer);
#endif

#ifndef END_MEASURE_BUILD
#define END_MEASURE_BUILD(timer) \
    stopTimer(&timer.simple_buildtimer);
#endif
...
#ifndef START_MEASURE_NP
#define START_MEASURE_NP(timer) \
    gettimeofday(&timer.start, NULL); \
    startTimer(&timer.overall_timer); \
    startTimer(&timer.progressivetimer[0]); \
    startTimer(&timer.progressivetimer[1]); \
