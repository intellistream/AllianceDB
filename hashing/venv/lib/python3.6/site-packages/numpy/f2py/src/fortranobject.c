...
void dump_attrs(const PyArrayObject* obj) {
    ...
    MSG("\trank = %d, flags = %d, size = %" NPY_INTP_FMT  "\n",
           rank,arr->flags,size);
    MSG("\tstrides = ");
...
PyArrayObject* array_from_pyobj(const int type_num,
    ...
    if (PyArray_Check(obj)) {
        ...
        /*
        MSG("intent alignment=%d\n", F2PY_GET_ALIGNMENT(intent));
        MSG("alignment check=%d\n", F2PY_CHECK_ALIGNMENT(arr, intent));
        int i;
        for (i=1;i<=16;i++)
...
#ifdef DEBUG_COPY_ND_ARRAY
    dump_attrs(arr);
    MSG("check_and_fix_dimensions:init: dims=");
    dump_dims(rank,dims);
#endif
...
#ifdef DEBUG_COPY_ND_ARRAY
    MSG("check_and_fix_dimensions:end: dims=");
    dump_dims(rank,dims);
#endif
    return 0;
}
