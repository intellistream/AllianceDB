...
def DrawLegend(legend_labels, filename):
    ...
    # figlegend = pylab.figure(figsize=(30, 0.5))
    ...
    for idx in range(len(FIGURE_LABEL)):
        # print(i)
        bars[idx] = ax1.scatter(0, 0, c = LINE_COLORS[idx], marker = MARKERS[idx])
...
########### sample
    ...
    for algo in hash_non_smp_lz:
        tmp_ls = []
        for gp in range(100, 120):
            file = exp_dir + '/results/breakdown/ALL_ON_Rovio_' + algo + '_profile_' + str(gp) + '_39.txt'
            with open(file, "r") as f:
                fc = f.read()
                tmp_ls.append(float(re.findall(r'Results = (.*?). DONE.', fc)[0]))
        tmp_ls = np.array(tmp_ls)
        col.append(np.std(tmp_ls, ddof = 0) / np.mean(tmp_ls))
    
    sort_non_smp_lz = ['m-pass','m-way']
    for algo in sort_non_smp_lz:
        tmp_ls = []
        for gp in range(100, 120):
            file = exp_dir + '/results/breakdown/Rovio_' + algo + '_profile_' + str(gp) + '_39.txt'
            with open(file, "r") as f:
                fc = f.read()
                tmp_ls.append(float(re.findall(r'Results = (.*?). DONE.', fc)[0]))
        tmp_ls = np.array(tmp_ls)
...
if __name__ == "__main__":
    ...
    legend_labels = [ 'S-NPJ', 'S-PRJ', 'S-MWAY', 'S-MPASS',
                     '', 'S-SHJ$^{JM}$', 'S-SHJ$^{JB}$', 'S-PMJ$^{JM}$', 'S-PMJ$^{JB}$',
                     '', 'PROBHASH$^{JM}$', 'PROBHASH$^{JB}$',
                     '', 'SRAJ$^{JM}$', 'SRAJ$^{JB}$',
                     '', 'PROBALL$^{JM}$', 'PROBALL$^{JB}$']
    ...
    DrawFigure(x_values, y_values, legend_labels,
               'Normalized Tpt. ', 'Coefficient Variance', 0,
               400, 'throughput_variance_tradeoff', False)

    DrawLegend(legend_labels, 'tradeoff_legend')
