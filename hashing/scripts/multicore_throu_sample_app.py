...
# SHJ_JBCR_NP
    ...
    'SHJ_JM_NP', 'SHJ_JBCR_NP','SHJ_JM_NP', 'SHJ_JBCR_NP',
    # 'SHJ_JM_NP', 'SHJ_JBCR_NP',
    ]
    task = ['_39', '_41', '_38']
    DataName = ['Rovio', 'DEBS', 'Stock']
    DataSize = [(2873604*2), (2e6), (1013800+1034443)]
    gp =[[4100, 4102, 4104, 4106, ]]*8 + [[4300, 4302, 4304, 4306,]]*2 + [[4200, 4202, 4204, 4206,]]*2 #  + [[4400, 4402, 4404, 4406,]]*2
    avx_size = [1,2,4,8]
    print(gp)
    for i in range(len(task)):
        average_thr = np.array([0]*len(avx_size))
        for idx, algo in enumerate(algo_list):
            ...
            for j in range(len(avx_size)):
                ...
                with open(file_name, "r") as f:
                    fc = f.read()
                    thr = DataSize[i]/(float(re.findall(r'99th latency: \((.*?)\)', fc)[0])+1000)
                    thr_list.append(thr)
            # print(thr_list)
            # if (np.max(thr_list)<10):
            #     thr_list = np.array([1.]*len(avx_size))
            # else:
            thr_list = np.array(thr_list)/np.min(thr_list)
            # print(thr_list)
            average_thr = average_thr +  thr_list
        x_axis.append(avx_size)
        y_axis.append(average_thr/(len(algo_list)))
        # print(average_thr)
        # print()
    # for i in y_axis:
    #     i[0] = 1
...
def DrawFigure(xvalues, yvalues, legend_labels, x_label, y_label, x_min, x_max, y_min, y_max, filename, allow_legend):
    ...
    # figure.xaxis.set_major_locator(LogLocator(base=2))
...
if __name__ == "__main__":
    ...
    DrawFigure(x_axis, y_axis, legend_labels,
            'Number of cores', 'Normalized 95th Throughput', 0, 0,
            1, 0,
            'multicore_sample_app_figure',
            legend)
