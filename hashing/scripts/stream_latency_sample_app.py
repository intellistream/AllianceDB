...
def ReadFile(id):
    ...
    if id =='AR':
        stream_attr = [1600, 3200, 6400, 12800, 25600]
    else:
        stream_attr = [0.0, 0.4, 0.8, 1.2, 1.6]

    sort_non_smp_lz = ['m-pass','m-way']
    for sort_algo in sort_non_smp_lz:
        col = []
        for idx, epsl in enumerate(stream_attr):
            tmp = 0 if id == 'AR' else 10
            file = exp_dir + "/results/breakdown/{}_{}_profile_{}_{}.txt".format(id, sort_algo, 4100, tmp+idx)
            with open(file, "r") as f:
                fc = f.read()
                col.append(float(re.findall(r'95th latency: \((.*?)\)', fc)[0]))
        coly = col
        x_axis.append(stream_attr)
        y_axis.append(coly)
    
    col = []
    for idx, epsl in enumerate(stream_attr):
        tmp = 0 if id == 'AR' else 10
        file = exp_dir + "/results/breakdown/ALL_ON_{}_{}_profile_{}_{}.txt".format(id,'NPO', 5100, tmp+idx)
        with open(file, "r") as f:
            fc = f.read()
            col.append(float(re.findall(r'95th latency: \((.*?)\)', fc)[0]))
            empty_col.append(0)
    ...
    for idx, epsl in enumerate(stream_attr):
        tmp = 0 if id == 'AR' else 10
        file = exp_dir + "/results/breakdown/ALL_ON_{}_{}_profile_{}_{}.txt".format(id,'PRO', 5100, tmp+idx)
        with open(file, "r") as f:
            fc = f.read()
            col.append(float(re.findall(r'95th latency: \((.*?)\)', fc)[0]))
    coly = col
    ...
    for hash_algo in hash_non_smp_eg:
        ...
        for idx, epsl in enumerate(stream_attr):
            tmp = 0 if id == 'AR' else 10
            file = exp_dir + "/results/breakdown/ALL_ON_{}_{}_profile_{}_{}.txt".format(id,hash_algo, 5100, tmp+idx)
            with open(file, "r") as f:
                fc = f.read()
                col.append(float(re.findall(r'95th latency: \((.*?)\)', fc)[0]))
        coly = col
...
# PROBEALL
    ...
    for hash_algo in hash_non_smp_eg:
        ...
        for idx, epsl in enumerate(stream_attr):
            tmp = 0 if id == 'AR' else 10
            file = exp_dir + "/results/breakdown/ALL_ON_{}_{}_profile_{}_{}.txt".format(id,hash_algo, 5300, tmp+idx)
            with open(file, "r") as f:
                fc = f.read()
                col.append(float(re.findall(r'95th latency: \((.*?)\)', fc)[0]))
        coly = col
...
# rsj
    ...
    for hash_algo in hash_non_smp_eg:
        ...
        for idx, epsl in enumerate(stream_attr):
            tmp = 0 if id == 'AR' else 10
            # file = exp_dir + "/results/breakdown/ALL_ON_{}_{}_profile_{}_{}.txt".format(id,hash_algo, 5400, tmp+idx)
            file = exp_dir + "/results/breakdown/ALL_ON_{}_{}_profile_{}_{}.txt".format(id,hash_algo, 5200, tmp+idx)
            with open(file, "r") as f:
                fc = f.read()
                col.append(float(re.findall(r'95th latency: \((.*?)\)', fc)[0]))
        # coly = np.array(col)*0.9
        coly = col
        x_axis.append(stream_attr)
        y_axis.append(coly)
...
def DrawFigure(xvalues, yvalues, legend_labels, x_label, y_label, x_min, x_max, y_min, y_max, filename, allow_legend):
    ...
    plt.xticks(x_values[0],x_values[0])
...
if __name__ == "__main__":
    ...
    for id in ['AR', 'AD']:
        ...
        tmp = ''
        if id == 'AR':
            tmp='$v_R=v_S$ (inputs/msec)'
        else:
            tmp='$skew_{ts}$'

        DrawFigure(x_axis, y_axis, legend_labels,
                tmp, '95th Latency (ms)', 0, 0,
                1, 0,
                'stream_latency_sample_app' + id,
                legend)
