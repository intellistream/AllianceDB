...
def DrawLegend(legend_labels, filename):
    ...
    LEGEND_FP = FontProperties(style='normal', size=10)
    ...
    for idx in range(len(FIGURE_LABEL)):
        # print(i)
        bars[idx] = ax1.scatter(0, 0, c = LINE_COLORS[idx], marker = MARKERS[idx], s = MARKER_SIZE)
    # LEGEND

    figlegend.legend(bars, FIGURE_LABEL, prop=LEGEND_FP, \
                     loc=1, ncol=len(FIGURE_LABEL)//2, mode="expand", shadow=True, \
                     frameon=True, handlelength=2, handletextpad=0.3, columnspacing=0.5,
                     borderaxespad=-0.2, fancybox=True
                     )
    figlegend.legend('boxoff')
    # figlegend.legend(bars, FIGURE_LABEL, prop=LEGEND_FP)
    # figlegend.legend(bars, FIGURE_LABEL, prop=LEGEND_FP)
...
if __name__ == "__main__":
    ...
    # for Datastr in [['Stock_', '_38', 1635926294], ['DEBS_', '_41', 251033140], ['Rovio_', '_39', 87856849382]]:
    for Datastr in [ ['Rovio_', '_39', 87856849382]]:
        x_values = ReadFile_latency()
        # x_values = ReadFile_latency_avg()
        # print(x_values)
        y_values = ReadFile_join_size()
        x_values = np.array(x_values)/np.max(x_values)
        y_values = np.array(y_values)/Datastr[2]
        y_values = list([list(y_values[i]) for i in range(len(y_values))])
        with open(Datastr[0] + 'joinsize.txt', 'w') as f:
    ...
    DrawFigure(x_values, y_values, legend_labels,
               'Normalized 95th latency', 'Normalized join size', 0,
               400, 'latency_join_size_tradeoff', False)

    DrawLegend(legend_labels, 'trade_off_legend')
