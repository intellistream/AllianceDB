...
def ReadFile():
    ...
    for Datastr in [['Rovio_', '_39'], ['DEBS_', '_41'], ['Stock_', '_38']]:
    ...
    ########### StrApp
        ...
        col.append(np.min(np.array(col[-16:-2])))
        col.append([])
        col.append([])
        col.append([])
...
if __name__ == "__main__":
    ...
    for Grpstr in GrpstrBuc:
        ...
        for i in range(len(cmp_ind[0])):
            # if ((y_values[cmp_ind[0][i]]-y_values[cmp_ind[1][i]])/y_values[cmp_ind[0][i]]) > 0.9:
            #     continue
            res.append((y_values[cmp_ind[0][i]]-y_values[cmp_ind[1][i]])/y_values[cmp_ind[0][i]])
