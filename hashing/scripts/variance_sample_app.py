...
COLOR_MAP = ('#B03A2E', '#2874A6', '#239B56', '#7D3C98', '#FFFFFF', '#F1C40F', '#F5CBA7', '#82E0AA', '#AEB6BF', '#AA4499', '#FFFFFF', '#AA4499', '#AA4499', '#FFFFFF', '#AA4499', '#AA4499', '#82E0AA', '#AEB6BF')
...
########### PROBE_ALL
    ...
    for id in it.chain(range(38, 42)):
        ...
        f = open(file, "r")
        read = f.readlines()
        x = float(read.pop(int(len(read) * 0.95)).algoip("\n"))  # get the 99th timestamp
        col1.append(x)
    y.append(col1)
...
if __name__ == "__main__":
    ...
    x_values = ['S-NPJ', 'S-PRJ', 'S-MWAY', 'S-MPASS',
                     '', 'S-SHJ$^{JM}$', 'S-SHJ$^{JB}$', 'S-PMJ$^{JM}$', 'S-PMJ$^{JB}$',
                     '', 'PROBHASH$^{JM}$', 'PROBHASH$^{JB}$',
                     '', 'SRAJ$^{JM}$', 'SRAJ$^{JB}$',
                     '', 'PROBALL$^{JM}$', 'PROBALL$^{JB}$',]
