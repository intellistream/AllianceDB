...
void VerifyEngine::Run(Context &ctx)
{
    ...
    for (size_t i = 0; i + param.window_length <= id_last_tuple; i += param.sliding_size)
    {  // walk through each window.
        auto r_end = std::min(i + param.window_length,
                              r_tuples.size());  // end of stream R of the current window.
        auto s_end = std::min(i + param.window_length,
                              s_tuples.size());  // end of stream S of the current window.
        std::unordered_map<KeyType, std::vector<TuplePtr>> r_map;
        for (auto j = i; j < r_end; j++)
        {  // walk through stream R of the current window.
            r_map[r_tuples[j]->key].push_back(r_tuples[j]);  // Construct the hashmap of streamR.
        }
        for (auto j = i; j < s_end; j++)
        {  // walk through stream S of the current window.
            auto &s_tuple = s_tuples[j];
            if (r_map.find(s_tuple->key) != r_map.end())
            {  // if there is a match.
                for (auto &r_tuple : r_map[s_tuple->key])
                {  // for each r_tuple that has the same key of s_tuple. This is to handle duplicate
                   // key case.
