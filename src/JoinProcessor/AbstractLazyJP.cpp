...
void AbstractLazyJP::inlineMain() {
 ...
 // NPJSingle algo;
  ...
  { //IDLE
    ...
    } else if(status==LWJ_PROCESSING)
    {  //cout<<"jp "+to_string(sysId)+"joins "+to_string(windowS.size())+","+to_string(windowR.size())<<endl;
      size_t sLen=windowS.size();
      size_t rLen=windowR.size();
      if(sLen&&rLen)
      { //add the cost model for algorithm decision in the future
