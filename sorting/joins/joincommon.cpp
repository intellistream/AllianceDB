...
sortmergejoin_initrun(relation_t *relR, relation_t *relS, joinconfig_t *joincfg,
    ...
    chunk_size[0] = relR->num_tuples / nthreads;
    chunk_size[1] = relS->num_tuples / nthreads;
    ...
    for (i = 0; i < nthreads; i++) {
        ...
        args[i].relR = relR->tuples + i * (chunk_size[0]);//logically partition the input relations..
        args[i].relS = relS->tuples + i * (chunk_size[1]);//logically partition the input relations..

        /* temporary relations */
        args[i].tmp_partR = tmpRelpartR + i * (chunk_size[0] + CACHELINEPADDING(PARTFANOUT));
        args[i].tmp_partS = tmpRelpartS + i * (chunk_size[1] + CACHELINEPADDING(PARTFANOUT));
        args[i].tmp_sortR = tmpRelsortR + i * (chunk_size[0]);
        args[i].tmp_sortS = tmpRelsortS + i * (chunk_size[1]);

        args[i].numR = (i == (nthreads - 1)) ?
                       (relR->num_tuples - i * chunk_size[0]) : chunk_size[0];
        args[i].numS = (i == (nthreads - 1)) ?
                       (relS->num_tuples - i * chunk_size[1]) : chunk_size[1];
