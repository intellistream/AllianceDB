...
struct T_TIMER {
    ...
    uint64_t overall_timer, partition_timer, sort_timer, mergedelta_timer, merge_timer, join_timer;
...
#ifndef BEGIN_MEASURE_SORT
#define BEGIN_MEASURE_SORT(timer) \
   startTimer(&timer.sort_timer);
#endif

#ifndef END_MEASURE_SORT
#define END_MEASURE_SORT(timer) \
   stopTimer(&timer.sort_timer);
#endif

#ifndef BEGIN_MEASURE_MERGEDELTA
#define BEGIN_MEASURE_MERGEDELTA(timer) \
   startTimer(&timer.mergedelta_timer);
#endif

#ifndef END_MEASURE_MERGEDELTA
#define END_MEASURE_MERGEDELTA(timer) \
   stopTimer(&timer.mergedelta_timer);
#endif

#ifndef BEGIN_MEASURE_MERGE
#define BEGIN_MEASURE_MERGE(timer) \
   startTimer(&timer.merge_timer);
#endif

#ifndef END_MEASURE_MERGE
#define END_MEASURE_MERGE(timer) \
   stopTimer(&timer.merge_timer);
#endif

#ifndef BEGIN_MEASURE_JOIN
#define BEGIN_MEASURE_JOIN(timer) \
   startTimer(&timer.join_timer);
#endif

#ifndef END_MEASURE_JOIN
#define END_MEASURE_JOIN(timer) \
   stopTimer(&timer.join_timer);
#endif
